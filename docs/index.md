...
heroText: 小曹同学的百草园
tagline: 一个普通的前端开发的博客。
# actionText: 立刻进入 →
# actionLink: /web/
bannerBg: '/img/bg-cover.jpeg' # auto => 网格纹背景(有bodyBgImg时无背景)，默认 | none => 无 | '大图地址' | background: 自定义背景样式       提示：如发现文本颜色不适应你的背景时可以到palette.styl修改$bannerTextColor变量
